...
- 搜索
  ...
  - 搜索框内置命令 (回车生效)
    - `@noise=n` 设置搜索宽容度（n 取值 0~5，默认 5）, 搜索词相邻两个字符之间，最多允许插入 n 个非空白杂字，设 2 以上搜索例如 "烟侍椅" 可命中 "烟枪侍女太师椅"
      - 搜索词长 (不含空格) 超过 `25` 降级为完全匹配 
    - `@like=n` 是否开启近义搜索（n 取值 0~1，默认 1 开启）,  设 1 后搜索例如 "他简直是秀逗了" 可命中 "他一定是糊涂了"
        - 近义词库取自哈工同义词林扩展版
